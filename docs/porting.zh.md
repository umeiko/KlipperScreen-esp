...
给仓库贡献一块新板子的支持，需要“一个 BSP 实现 + 全链路登记”。必须从 [`templates/board/`](https://github.com/umeiko/KlipperScreen-esp/tree/main/templates/board) 开始；现有 BSP 只用于查阅某个控制器的特殊实现，不作为复制底板。
...
4. 按本指南先做不带 LVGL 的五色色带测试，再验证开机动画、完整 UI 和声明的全部输入
